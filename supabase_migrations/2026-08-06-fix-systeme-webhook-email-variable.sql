-- Remove the PL/pgSQL ambiguity between the customer email variable and
-- public.users.email while preserving the launch migration's behavior.
...
create index if not exists storysculpt_projects_user_idx on public.storysculpt_projects (user_id);
create index if not exists solution_vault_progress_item_idx on public.solution_vault_progress (item_id);
