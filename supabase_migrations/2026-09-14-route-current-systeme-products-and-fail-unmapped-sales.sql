-- Route the current public offers and make an unmapped sale visible as a failure.
-- Existing failed or ignored deliveries can be retried after their route is fixed.

insert into public.systeme_product_routes (price_plan_id, product_key, app_keys, active)
values
...
begin
  ...
  if not found then
    ...
    from public.systeme_webhook_events
    ...
    if prior_status = 'processed' then
  ...
  if not found then
    ...
    set status = 'failed', processed_at = now(),
        result = jsonb_build_object('reason', 'unmapped_price_plan', 'price_plan_id', plan_id)
    where message_id = p_message_id;
    return jsonb_build_object('failed', true, 'reason', 'unmapped_price_plan', 'price_plan_id', plan_id);
  end if;
