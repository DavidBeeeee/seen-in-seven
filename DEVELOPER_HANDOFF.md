...
Both production and the Prompt Tester now use the same prompt focusing, context assembly, output validation, and final-script assembly. A malformed response gets up to two narrowly instructed repair attempts. Validation checks all five labels and also rejects repeatable architecture failures: an overlong Open Loop, an Open Loop that announces the realization or uses vague "what happened next" suspense, a CTA that begins with a series label instead of bridging from the Conclusion, a CTA that puts challenge or series orientation into the bridge sentence, a bare future video number that does not explain the 7 Video Challenge to a cold viewer, a CTA without one explicit viewer action, a CTA without "because" connecting the action to its reason, and a CTA that identifies the current installment with the wrong number. The repair request names the exact failure while preserving the supplied facts, voice, and story beats. For an overlong Open Loop it also reports the measured word count and requires a 35-45 word replacement with a hard maximum of 50. If the first repair still fails, the final precision pass repeats the measured requirement instead of immediately returning an error to the user. Video 1's prewritten challenge declaration is inserted between Open Loop and Meat in the canonical final script used for display, copy, PDF, database `final_content`, and future-video context. Section regeneration uses the same focused system prompt and cumulative context as full generation.
