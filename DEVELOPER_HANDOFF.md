...
- Each option is now a complete answer plan rather than one fragment of a multi-part direction. A private coverage pass preserves every direction claim, accounts for every current question, and develops missing material from context before the three options vary their scene, evidence, interpretation, or emotional emphasis.
- Options may use one compact paragraph of up to 100 words instead of the former two-sentence cap, which had encouraged outside AIs to divide one story across three incomplete choices. Level 2 Video 1 explicitly includes why remaining quiet stopped feeling acceptable now.
...
- The Journey helper also checks that each direction answers every part of its multi-part question rather than satisfying one clause and silently dropping the rest.
