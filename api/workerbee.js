...
const ACTIONS = new Set([
  ...
  'create_task_note', 'acknowledge_task_note',
...
const TASK_NOTE_FIELDS = new Set(['id', 'task_id', 'body']);
...
function sanitize(action, input) {
  ...
  else if (action.includes('task_note')) payload = pick(input, TASK_NOTE_FIELDS);
  ...
  if (action === 'create_task_note') {
    payload.task_id = cleanText(payload.task_id, 64);
    if (!payload.task_id) throw new Error('A note has to belong to a task.');
