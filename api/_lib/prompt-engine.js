...
function publishedPrompt() {
  ...
  const STAGE_CONTRACTS = {
    ...
    7: 'RETURN. Build one causal present-day scene: an observable action creates a consequence that triggers one callback, the contrast explains a changed choice, and one unfinished flaw complicates that same action. Derive one earned transformation from the audience canon, reserve its peer-to-peer gift for the Conclusion, and open an ongoing relationship. Do not arrange these as separate declarations, recap the journey, discover another truth, mention every earlier chapter, explain why the speaker deserves followers, or introduce an offer.'
  };
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Video 7 at either level, require one causal present-day Return scene. The observable action must produce the consequence that triggers the callback; that contrast must explain the changed choice; and the unfinished flaw must complicate the same action rather than open a separate topic. Reject a thematic essay, list of convictions, retrospective summary, or series of movable Return declarations even when every required ingredient appears. Require the speaker\'s transformation to become visible through what they do before the script interprets what it means. The Conclusion must answer the exact Open Loop through what this scene proves about the changed but unfinished speaker, then turn that earned recognition outward as one specific peer-to-peer gift. Reject another epiphany, universal slogan, or unresolved experiment as the ending.',
    'For Level 2 Video 7, leave the HOOK to the global Hook Studio with no required present-day action or return beat. Require one present-day scene whose current action causes the callback, whose callback changes the next choice, and whose unfinished flaw complicates the same unfolding consequence. Reject a MEAT assembled from movable beliefs, lessons, industry claims, professional comparisons, or Return ingredients even when they share a topic or appear in a logical order. The transformation must be visible in behavior before it is interpreted. Use only one full-circle callback and reject episode-by-episode recap, professional superiority, positioning, a market manifesto, or an unresolved business experiment as the center of the Return. Keep the unfinished element human and the continuing direction relational rather than commercial.',
