...
function publishedPrompt() {
  ...
  const STAGE_CONTRACTS = {
    ...
    7: 'RETURN. Bring the six-part audience canon home through one governing identity transformation that a cold viewer can understand and a returning viewer can recognize. Preserve the earlier identity and refusal, first realization and trial, fall, elixir, and returned self while using only the strongest selected evidence for each movement. Expand the selected causal spine through complete Hook-and-Eye prose instead of restoring every episode event or compressing connective logic. Reveal the integrated lesson, earned difference, peer-to-peer gift, honest remainder, and relational horizon. Do not create a new central conflict, trial, revelation, unresolved experiment, pitch, or offer.'
  };
  ...
  function wholeScriptRewriteMessage(config, script, review, validation) {
    const compositionInstruction = Number(config.video) === 7
      ? 'Apply sentence-level Hook-and-Eye only inside [MEAT]. Preserve every transformation and only the evidence selected inside CONNECTED JOURNEY PROGRESSION, then expand that causal spine into complete spoken thoughts without restoring omitted canon or compressing connective logic. Reserve RESERVED RETURN for [CONCLUSION], preserve HONEST REMAINDER AND HORIZON, keep [OPEN LOOP] independent from the Hook, and do not imitate wording from the draft below.'
