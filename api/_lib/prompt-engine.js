...
function publishedPrompt() {
  ...
  function stageContract(level, video) {
    ...
    if (Number(video) === 6) {
      return base + ' LEVEL 2: Build the professional elixir through earned conviction. Require the causal chain from the specific Video 5 fall, through the aftermath evidence and the pre-fall professional understanding that can no longer explain it, into one observable changed decision and one carryable truth. Treat Video 3 as optional continuity evidence only when the supplied story naturally connects the two epiphanies. Reject a forced Video 3 correction, an unrelated hot take, a repetition of Video 3, a pre-existing business philosophy, or an offer disguised as the lesson.';
    }
    if (Number(video) === 7) {
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Video 6 at either level, leave the HOOK to the global Hook Studio after the story is settled. It receives no required fall evidence, verdict, question, or story beat, though it may be more provocative or convicted because the speaker has earned that tone. It cannot reveal the elixir or perform the Open Loop. Require one exact conceptual question in the OPEN LOOP about the missing meaning created by the Video 5 fall and its aftermath, then verify that the CONCLUSION answers that same question. The MEAT must carry one coherent aftermath evidence thread, end with the lived evidence irreconcilable under the speaker\'s pre-fall understanding, and contain no statement or paraphrase of the reserved elixir. The CONCLUSION must state one complete hard-won paradigm shift for the first time, then one new possibility or human consequence. A Video 3 connection is optional and must never be manufactured.',
    ...
    'For Level 2 Video 6, ground the Epiphany in the specific fall and its aftermath without stating the elixir early. Require an explicit causal chain: Video 5 exposed a failure in the speaker\'s pre-fall professional understanding; aftermath evidence intensifies the cognitive dissonance; changed behavior supports the hard-won truth; and the CONCLUSION resolves the exact Open Loop with one complete carryable professional elixir. Preserve a Video 3 relationship only when the supplied story genuinely supports it. Reject a forced correction of Video 3, an unrelated industry opinion, a repetition of Video 3, generic wisdom that did not require the fall, a pre-existing business philosophy, a method list, service description, pricing structure, or pitch.',
