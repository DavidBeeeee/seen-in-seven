...
function publishedPrompt() {
  ...
  function validateOutput(text, video, level, userContext = '') {
    ...
    if (Number(level) === 2 && Number(video) === 4) {
      if (/\b(?:can(?:not|'t)|could(?: not|n't)|did(?: not|n't)|would(?: not|n't))\s+(?:solve|survive|handle|withstand|carry|fix|prevent)\b|\b(?:the\s+fall|collapse[ds]?|failure|fails?|breaks?)\b/i.test(cta)) {
        addIssue('CTA', 'Level 2 Video 4 CTA predicts or reveals the coming ordeal. Continue from the first meaningful result and the future it made possible without saying something will fail, collapse, break, or prove insufficient.');
      }
      ...
      Object.keys(sections).forEach(section => {
        ...
        if (futureNarratorDiagnosis.test(sectionText) || /\b(?:warning(?:\s+sign)?|red flag|blind spot|signal)\b/i.test(sectionText)) {
          addIssue(section, 'Level 2 Video 4 uses later hindsight to diagnose the recoverable trial. Keep the speaker inside the uncertainty, choice, and first hope available then. Do not foreshadow or explain Video 5.');
        }
      });
    }
    if (Number(video) === 7) {
  ...
  function stageContract(level, video) {
    ...
    if (Number(video) === 4) {
      return base + ' LEVEL 2: The real arena may be work, craft, calling, business, life, or public communication. Do not assume that making videos is the whole road of trials. Center the recognizable person choosing what they believe while the old way still appears to be winning. Keep the trial recoverable, reserve the meaningful result as the exact answer to the Open Loop until the Conclusion, and frame it as possibility rather than professional proof. Do not diagnose or foreshadow the fall.';
    }
    if (Number(video) === 6) {
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Level 2 Video 4, require one coherent recoverable-trial sequence: a brief first lens, observable changed action, a recognizable old-world temptation, a choice made while the outcome is unknown, and one supported meaningful result. The recoverability boundary is mandatory: reject completed collapse, apparently permanent loss, failed recovery, worst-day framing, or any event the speaker could not simply try again after; those belong to Video 5. The emotional center must be the speaker\'s uncertainty and choice while the old way appears to be winning, not professional superiority, market analysis, a case study, or proof of a method. The HOOK is a pure pattern interrupt built from RECOVERABLE TRIAL or OLD-WORLD TEMPTATION. Require one exact pressing question in the OPEN LOOP whose reserved answer is FIRST MEANINGFUL RESULT; reject general setup, decision summaries, statements of stakes, vague anticipation, and any early statement or implication of the result. The MEAT must stop before the result. The CONCLUSION must answer the exact Open Loop with that result for the first time, then state only what it made possible to the speaker then. WHAT REMAINED OPEN is optional and cannot be manufactured into a warning or failure seed. Reject a repeated Video 3 lesson, universal advice, present-day interpretation, offers, methods, challenge recaps, content-progress reports, second epiphanies, hindsight diagnoses, and CTAs that predict Video 5. The CTA reason must continue a specific consequence of the result, not generic commentary about the next challenge, real test, middle of the story, or what happens next. Compare every precise duration, count, amount, reaction, and result against the curated material; reject invented precision or unsupported proof. If the result is absent, preserve the unanswered question rather than inventing an ending.',
    'For Level 2 Video 5, enforce the one-way-door ordeal standard. The event must be different in kind from Video 4\'s recoverable pressure: something central was actually destroyed, ended, lost, or believed impossible to restore, the speaker\'s consequential choice materially contributed, and an attempted way back failed. Briefly establish what had become real enough to lose, then build the opening from concrete collapse evidence and causation and end inside owned apparent permanent loss. Reject low attention, comparison, embarrassment, uncertainty, ordinary disappointment, weak performance, or visibility anxiety dressed in catastrophic adjectives. Reject recovery, reassurance, lessons, silver linings, redemption, or elixir language.',
