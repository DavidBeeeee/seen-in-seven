...
function publishedPrompt() {
  ...
  const STAGE_CONTRACTS = {
    ...
    3: 'GUIDE + FIRST EPIPHANY + THRESHOLD. A guide influence helps expose what the speaker could not see from inside the old understanding, and lived evidence makes the old understanding break. Deliver one complete but bounded paradigm shift that gives the viewer a powerful usable lens. Do not weaken the first epiphany merely because a deeper elixir comes later; reserve only the truth that requires the fall, a complete method, and mature authority.',
  ...
  function stageContract(level, video) {
    ...
    if (Number(video) === 3) {
      return base + ' LEVEL 2: Build one complete but bounded professional paradigm shift through cognitive surprise. Infer it from one guide\'s usable contribution, one representative evidence scene, the old assumption, and its human cost. The guide must be a literal person, but a body of work or teaching may provide the lens when described honestly. Name only that one guide or source figure anywhere in the script. Do not list mentors as credentials, turn the discovery into an industry lecture, copy a complete method or pricing philosophy from the answers, import the mature elixir, or frame the conclusion as what the industry should do. The speaker receives and tests guidance here; they do not become the viewer\'s guide yet.';
    }
    if (Number(video) === 4) {
      return base + ' LEVEL 2: The real arena may be work, craft, calling, business, life, or public communication. Do not assume that making videos is the whole road of trials. Keep the speaker inside what they reasonably believed at that point in time. Let the audience notice one unresolved fact, but do not diagnose it with hindsight, reveal the fall, or turn the trial into another epiphany.';
    }
    if (Number(video) === 6) {
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Video 3 at either level, require a FORBIDDEN-IDEA PROMISE in the HOOK. It must signal that an accepted explanation, respectable assumption, or common measurement is incomplete, backward, or socially protected and promise a consequential new lens without disclosing that lens. Reject scene-first evidence hooks, soft confessions, old-belief summaries, progress reports, generic industry complaints, reusable claims that everyone is wrong, and hooks that only become interesting after personal context. The OPEN LOOP must establish why the accepted idea seemed reasonable and name the specific contradiction or human cost it cannot explain. The MEAT must carry the discovery arc and stop before stating the reframe. The CONCLUSION must deliver one complete but bounded paradigm shift, transfer the lens to the viewer, and make the old interpretation\'s cost newly visible.',
    'For Video 6 at either level, require an EARNED VERDICT in the HOOK. It must state a sharp paradox, failed rule, or protected belief that the Video 5 fall gives the speaker the right to challenge while withholding the elixir\'s deeper causal truth. Reject aftermath-scene hooks, generic hot takes, and early elixir reveals. The OPEN LOOP must name the collision between the true Video 3 lens and the fall it could not explain or prevent. The MEAT must establish the causal chain and stop before stating the elixir. The CONCLUSION must deliver one complete deeper paradigm shift, transfer it to the viewer, and make the new possibility visible.',
    ...
    'For Level 2 Video 3, require one mentor-assisted professional paradigm shift built through cognitive surprise rather than a polished thought-leadership lecture. One literal person must provide a specific question, correction, example, permission, warning, teaching, source, demonstration, or reference point; a body of work may supply the guide lens when attributed honestly without inventing a direct conversation. Name only that one guide, mentor, teacher, or source figure anywhere in the script. Reject any second mentor name, credential list, or self-authority claim based on knowing several teachers or frameworks. Require one representative scene or occurrence where the old assumption stops matching reality, but keep that scene in the MEAT rather than using it as the default HOOK. The speaker must receive and test the lens before the CONCLUSION delivers one complete but bounded truth. The CONCLUSION must reveal a hidden relationship, cause, category error, or reversal; reject a recommendation about what the industry, price, method, or people should do. Reject multiple reframes, pricing ladders, complete methods, current service descriptions, mature business philosophy, offers, and anything that belongs to the Video 6 elixir or Video 7 return.',
    'For Level 2 Video 4, require a real sequence containing changed action, at least two meaningful pressures, decisions, or applications, a visible partial win, and the reason it created confidence in the actual subject of the larger story. The first epiphany or guide may receive one brief continuity reference, never a recap or another lesson. The HOOK must come only from HIGHEST-FRICTION TRIAL in the curated packet and enter its conflict, difficult choice, contradiction, resistance, or social pressure already in motion. Reject a setup-style first application when stronger friction is supplied. The OPEN LOOP must continue that same trial, identify only its immediate stakes, and withhold its result and meaning; reject explanations of why the old approach was wrong or what another person learned. Keep the CONCLUSION inside what the speaker reasonably believed then: place one concrete unresolved fact beside earned confidence so the audience may sense instability without the speaker diagnosing it. Reject present-day interpretation, offers, methods, challenge recaps, content-progress reports, second epiphanies, premature fall explanations, and CTAs that predict or reveal the coming failure. The CTA reason must continue a specific confidence, decision, or unresolved pressure from the conclusion, not generic commentary about the middle of a story or a real test. Compare every precise duration, count, amount, reaction, and result against the curated material; reject invented precision or unsupported proof. If a trial outcome is absent from the curated packet, the script must not complete it. Reject future-narrator labels such as warning, signal, blind spot, red flag, or a claim that the speaker failed to notice something then.',
    ...
    'For Level 2 Video 6, ground the EARNED VERDICT in the specific fall and its aftermath without narrating the aftermath scene in the opening or stating the elixir early. Require an explicit causal chain: Video 3 supplied a useful first lens; Video 5 proved what that lens could not solve; aftermath evidence forced a deeper interpretation; changed behavior proves the new truth; and the CONCLUSION states one complete carryable professional elixir. Reject an unrelated industry opinion, a repetition of Video 3, generic wisdom that did not require the fall, a pre-existing business philosophy, a method list, service description, pricing structure, or pitch.',
