...
function publishedPrompt() {
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Level 2 Video 3, require one raw first epiphany built through cognitive surprise rather than a polished thought-leadership lecture. One literal person must provide a specific question, correction, example, permission, warning, teaching, source, demonstration, or reference point; a list of mentor names or credentials does not satisfy this. A body of work may supply the guide lens when attributed honestly without inventing a direct conversation. Require one representative scene or occurrence where the old assumption stops matching reality. Build the opening from that evidence, not the final reframe. The speaker must receive and test the lens before the CONCLUSION delivers one narrow, carryable first truth. Reject multiple reframes, pricing ladders, complete methods, current service descriptions, mature business philosophy, offers, and anything that belongs to the Video 6 elixir or Video 7 return.',
