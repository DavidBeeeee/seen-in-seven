...
function publishedPrompt() {
  ...
  const STAGE_CONTRACTS = {
    ...
    7: 'RETURN. Reprise one concrete pressure, situation, or behavior the audience already heard, then show its present-day echo and the speaker\'s different response. Build one causal scene: the familiar pressure triggers the callback, the callback makes the earlier response visible, the changed choice produces a human consequence, and one residual emotion or temptation proves the speaker remains unfinished. Carry one already-earned transformation outward as the peer-to-peer gift. Do not create a new central conflict, trial, loss, revelation, or unresolved experiment; arrange Return ingredients as separate declarations; recap the journey; explain why the speaker deserves followers; or introduce an offer.'
  };
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Video 7 at either level, require one causal Return scene built from an echo of a concrete pressure, situation, or behavior the audience already heard in Videos 1 through 6. A generic callback to permission, confidence, visibility, belonging, failure, or another broad theme does not qualify. The familiar pressure must make the earlier response visible; the present speaker must choose differently under that same kind of pressure; the changed choice must create a human consequence; and one residual emotion or temptation must remain without controlling the decision. Reject a new central conflict, public test, dramatic loss, future trial, unresolved external outcome, business experiment, or fresh revelation. Reject a thematic essay, list of convictions, retrospective summary, or series of movable Return declarations even when every required ingredient appears. Require the transformation to become visible through behavior before interpretation. The Conclusion must answer the exact Open Loop through what this reprise proves about the changed but unfinished speaker, then carry an already-earned truth outward as one specific peer-to-peer gift.',
    'For Level 2 Video 7, leave the HOOK to the global Hook Studio with no required present-day action or return beat. Treat the professional subject as evidence inside the human Return, never as the governing story. Reject a MEAT assembled from movable beliefs, lessons, industry claims, professional comparisons, or Return ingredients even when they share a topic or appear in a logical order. Use only one concrete full-circle callback and reject episode-by-episode recap, professional superiority, positioning, a market manifesto, or an unresolved business experiment as the center of the Return. Keep the unfinished element as a residual human feeling, habit, fear, or temptation and keep the continuing direction relational rather than commercial.',
