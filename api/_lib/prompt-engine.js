...
function publishedPrompt() {
  ...
  function buildUserMessage(config) {
    ...
    const sourceOwnership = video === 7
      ? [
          ...
          '- Bring the earlier identity, decisive shifts, fall, elixir, returned self, and unfinished flaw together as one connected transformation rather than a present-day scene or episode-by-episode recap.',
  ...
  const STAGE_CONTRACTS = {
    ...
    7: 'RETURN. Bring the six-part audience canon home as one connected transformation that a cold viewer can understand and a returning viewer can recognize. Move from the earlier identity and refusal through the decisive realization, trial, fall, elixir, and returned self without announcing or separately summarizing each episode. Reveal the integrated lesson, earned difference, peer-to-peer gift, honest remainder, and relational horizon. Do not create a new central conflict, trial, revelation, unresolved experiment, pitch, or offer.'
  };
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Video 7 at either level, require one connected Return across the audience canon. A cold viewer should understand the earlier person and governing bind, the decisive realization and trial, the fall and human cost, the elixir, and the returned but unfinished self; a returning viewer should recognize concrete details from the journey. The Meat may compress chapters, but it cannot reduce the Return to one new or present-day scene, announce episode numbers, summarize installments separately, repeat every earlier lesson, or arrange retrospective declarations that could move without breaking the flow. Preserve independent epiphanies as independent sources when the canon requires it. The Conclusion must reveal the integrated lesson and returned identity for the first time, carry one specific peer-to-peer gift outward, acknowledge the honest remainder, and open a continuing relationship.',
    'For Level 2 Video 7, leave the HOOK to the global Hook Studio with no required return beat. Require the professional difference to emerge from the complete lived journey: the specific perspective, practice, or way of seeing this speaker can now carry that a more ordinary route would not. Earn that difference through canon evidence instead of generic superiority, positioning, a market manifesto, or a pitch. The speaker may now guide the person they once resembled, while the unfinished flaw keeps the authority human and the continuing direction relational rather than commercial.',
  ...
  function wholeScriptRewriteMessage(config, script, review, validation) {
    const compositionInstruction = Number(config.video) === 7
      ? 'Apply sentence-level Hook-and-Eye only inside [MEAT]. Build MEAT exclusively from CONNECTED JOURNEY PROGRESSION as one seamless retrospective journey a cold viewer can understand and a returning viewer can recognize. Reserve RESERVED RETURN for [CONCLUSION], preserve HONEST REMAINDER AND HORIZON, keep [OPEN LOOP] independent from the Hook, and do not imitate wording from the draft below.'
