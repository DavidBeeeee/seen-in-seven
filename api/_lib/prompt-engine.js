...
function publishedPrompt() {
  ...
  const STAGE_CONTRACTS = {
    ...
    7: 'RETURN. Bring the six-part audience canon home through one governing identity transformation that a cold viewer can understand and a returning viewer can recognize. Treat EARLIER SELF from Videos 1-2, FIRST SHIFT from Videos 3-4, FALL from Video 5, and RETURN from Video 6 plus current Video 7 evidence as the complete lived-evidence budget. MEAT performs the complete Return through Hook-and-Eye prose, lets the earned difference emerge from behavior, and ends with the honest unresolved element. CONCLUSION is only a brief one-to-three-sentence homecoming statement of the governing meaning, written entirely in I/me/my with no second-person you/your language, recap, evidence, viewer diagnosis, or second miniature Return. CTA alone carries direct viewer recognition and the relational horizon. Do not create a second thesis, new conflict, trial, revelation, service promise, superiority comparison, pitch, or offer.'
  };
  ...
  function stageContract(level, video) {
    ...
    if (Number(video) === 7) {
      return base + ' LEVEL 2: Make the speaker\'s specific professional difference emerge from the lived journey and show why that perspective now belongs in public. Carry the earned perspective back as recognition shared between equals, never as a promise to teach, show, guide, coach, rescue, transform, or produce a result. The unfinished flaw keeps the close human and relational rather than commercial.';
    }
    return base;
  }
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Video 7 at either level, require one connected Return organized around a single governing identity transformation. A cold viewer should understand the earlier person and refusal from Videos 1-2, the actual first realization and trial from Videos 3-4, the fall and human cost from Video 5, and the elixir and returned but unfinished self from Video 6 plus current Video 7 evidence; a returning viewer should recognize the defining callbacks. Treat those four private anchors as the complete canon evidence budget. MEAT owns the complete Return. Each Meat anchor must remain a lived event, choice, consequence, or observable behavior rather than a lesson, identity declaration, positioning claim, service description, or superiority comparison. Let the earned identity or professional difference emerge from the behavior and require MEAT to end with the honest unresolved human element. Reject any unselected Meat evidence, archive summary, paragraph-per-video recap, overloaded chronology, movable retrospective declarations, a Return reduced to one present-day scene, or compressed prose that drops causal connection. Preserve independent epiphanies as independent sources when the canon requires it. CONCLUSION is only a brief homecoming statement of one to three sentences that answers the Open Loop with the governing meaning in RESERVED RETURN. Require the Conclusion to remain entirely in I/me/my, reject second-person you/your language or any attempt to address, interpret, diagnose, or advise the viewer there, and reserve direct viewer recognition for CTA. Reject any Conclusion that replays an anchor event, names or summarizes the four movements, restates the earned difference, includes the honest remainder, adds evidence, introduces another lesson, or performs a second miniature Return. CTA alone carries the relational horizon.',
    'For Level 2 Video 7, leave the HOOK to the global Hook Studio with no required return beat. Require the professional difference to emerge from the complete lived journey: the specific perspective, practice, or way of seeing this speaker can now carry that a more ordinary route would not. Earn that difference through selected canon evidence instead of generic superiority, positioning, a market manifesto, or a pitch. Carry the gift as recognition between equals while the unfinished flaw keeps the authority human and the continuing direction relational rather than commercial.',
    ...
    'Inside MEAT, logical ordering alone does not satisfy Hook-and-Eye. Each sentence must arise from the sentence immediately before it through cause, consequence, time, escalation, contradiction, or changed action. Reject a chain of movable declarations that merely share a topic or follow the architecture beats in order. Do not apply this prose-flow test to HOOK or OPEN LOOP.',
    ...
    'For Video 7, require a relational close: acknowledge the completed Video 7 of 7 arc, ask the viewer to follow primarily because they recognize, trust, or want to stay connected to this person and perspective, and invite late viewers back to Video 1. An unfinished public test may add supporting tension, but it cannot be the only reason to follow. Do not imply Video 8, invent urgency, or introduce an offer.',
  ...
  const OPEN_LOOP_WRITER_SYSTEM = [
    ...
    'Keep the speaker in the same first-person voice used by the visible story. Never refer to the speaker as he, she, or they, and never turn the retention gap into an editor describing the character from outside.',
    'Create one unresolved mental relationship. Do not stack two separate questions or ask the viewer to wait for two different answers.',
  ...
  const HOOK_STUDIO_SYSTEM = [
    ...
    'Create fresh Hook language. Never copy a complete sentence or an eight-word sequence from the Open Loop, Meat, Conclusion, or CTA.',
  ...
  const HOOK_JUDGE_SYSTEM = [
    ...
    'Fail a candidate that copies a complete sentence or an eight-word sequence from the visible story. Judge its attention mechanism independently, but require fresh language.',
  ...
  async function generateFinalHook(config) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      const validCandidates = candidates.filter(hook => {
        ...
        const repeatsVisibleStory = ['OPEN LOOP', 'MEAT', 'CONCLUSION', 'CTA']
          .some(section => repeatedSectionPhrase(hook, sections[section]));
        return !hookIssues.length && !repetitionIssues.length && !repeatsVisibleStory;
      });
  ...
  function wholeScriptRewriteMessage(config, script, review, validation) {
    const compositionInstruction = Number(config.video) === 7
      ? 'Apply sentence-level Hook-and-Eye only inside [MEAT]. Treat EARLIER SELF, FIRST SHIFT, FALL, and RETURN as the complete lived-evidence budget, then develop those anchors into the complete Return without restoring omitted canon, replacing evidence with positioning, or compressing connective logic. Let the earned difference emerge inside [MEAT] and end [MEAT] with the honest unresolved element. Reserve the one governing meaning in RESERVED RETURN for a one-to-three-sentence [CONCLUSION] written entirely in I/me/my without you/your language, event replay, recap of the four movements, viewer interpretation, added evidence, or another miniature Return. Reserve direct viewer recognition and the relational horizon for [CTA], keep [OPEN LOOP] independent from the Hook, and do not imitate wording from the draft below.'
      : 'Apply sentence-level Hook-and-Eye only inside [MEAT]. A shared topic or correct chronology does not count: each Meat sentence must arise from the sentence immediately before it through cause, consequence, time, escalation, contradiction, or changed action. Do not paraphrase the architecture beats as a chain of movable declarations. Rebuild the standalone viewer premise near the beginning of MEAT from the current Journey Direction or Viewer Premise Source, then advance rather than restating it. Keep [OPEN LOOP] independent from the Hook and do not imitate wording from the draft below.';
    return [
      String(config.userMessage || '').trim(),
      '',
