...
function publishedPrompt() {
  ...
  const STAGE_CONTRACTS = {
    ...
    7: 'RETURN. Bring the six-part audience canon home through one governing identity transformation that a cold viewer can understand and a returning viewer can recognize. Treat EARLIER SELF, FIRST SHIFT, FALL, and RETURN from the private synthesis as the complete evidence budget. Develop those four anchors through complete Hook-and-Eye prose without restoring any omitted canon event, compressing connective logic, or exceeding the global 240-360 word pacing range merely to preserve more examples. Reveal the integrated lesson, earned difference, peer-to-peer gift, honest remainder, and relational horizon. Do not create a new central conflict, trial, revelation, unresolved experiment, pitch, or offer.'
  };
  ...
  function stageContract(level, video) {
    ...
    if (Number(video) === 7) {
      return base + ' LEVEL 2: Make the speaker\'s specific professional difference emerge from the lived journey and show why that perspective now belongs in public. The speaker returns as a guide through earned perspective, while the unfinished flaw keeps the close human and relational rather than commercial.';
    }
    return base;
  }
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Video 7 at either level, require one connected Return organized around a single governing identity transformation. A cold viewer should understand the earlier person and refusal, first realization and trial, fall and human cost, elixir, and returned but unfinished self; a returning viewer should recognize the defining callbacks. Treat EARLIER SELF, FIRST SHIFT, FALL, and RETURN from the private synthesis as the complete canon evidence budget. Reject any Meat event, number, person, example, comparison, or metaphor not selected inside those four anchors. Reject an archive summary, a paragraph-per-video recap, overloaded chronology, a visible script that exceeds the global pacing range merely to preserve more evidence, or any detail whose removal leaves the transformation unchanged. Also reject a Return reduced to one new or present-day scene, movable retrospective declarations, or compressed prose that drops the causal connection between selected moments. Preserve independent epiphanies as independent sources when the canon requires it. The Conclusion must reveal the integrated lesson and returned identity for the first time, carry one specific peer-to-peer gift outward, acknowledge the honest remainder, and open a continuing relationship.',
    'For Level 2 Video 7, leave the HOOK to the global Hook Studio with no required return beat. Require the professional difference to emerge from the complete lived journey: the specific perspective, practice, or way of seeing this speaker can now carry that a more ordinary route would not. Earn that difference through selected canon evidence instead of generic superiority, positioning, a market manifesto, or a pitch. The speaker may now guide the person they once resembled, while the unfinished flaw keeps the authority human and the continuing direction relational rather than commercial. Reject a CTA that asks the viewer to monitor whether the speaker proves or tests an ideal, model, experiment, or business result.',
    ...
    'For Video 7, require a relational close: acknowledge the completed Video 7 of 7 arc, ask the viewer to follow because they want to stay connected to this person and perspective, and invite late viewers back to Video 1. Reject follow reasons that merely ask the viewer to verify whether the speaker succeeds, prove a claim, audit an experiment, or watch a business model work. Do not imply Video 8, invent urgency, or introduce an offer.',
  ...
  function wholeScriptRewriteMessage(config, script, review, validation) {
    const compositionInstruction = Number(config.video) === 7
      ? 'Apply sentence-level Hook-and-Eye only inside [MEAT]. Treat EARLIER SELF, FIRST SHIFT, FALL, and RETURN as the complete evidence budget, then develop those anchors into complete spoken thoughts without restoring omitted canon or compressing connective logic. Keep the complete visible script inside the global 240-360 word pacing range by removing redundant evidence rather than shortening thoughts. Reserve RESERVED RETURN for [CONCLUSION], preserve HONEST REMAINDER AND HORIZON, keep [OPEN LOOP] independent from the Hook, and do not imitate wording from the draft below.'
