...
function publishedPrompt() {
  ...
  function stageContract(level, video) {
    ...
    if (Number(video) === 2) {
      return base + ' LEVEL 2: Identification comes before admiration. The ordinary human life, familiar identity, and understandable reason for staying must dominate. Future expertise is only a quiet clue. Reject hooks built from payment, clients, business milestones, professional recognition, or epiphany; hidden-expert origin stories; refusal that is merely named instead of shown through behavior; service philosophy; offers; current positioning; guru language; or a clean explanation of what the origin became.';
    }
    if (Number(video) === 3) {
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Level 2 Video 2, identification comes before admiration. Require a recognizable ordinary human life, a familiar identity the speaker could not imagine leaving, a quiet private thread, and an observable choice, delay, dismissal, or retreat that keeps them there. Reject a hidden-expert origin story, a hook built from payment, clients, business milestones, professional recognition, or epiphany, and any refusal that is only named retrospectively instead of shown through behavior. Enforce a present-day interpretation embargo: reject explanations of what the thread truly meant, why it qualified as expertise, or how it became the speaker\'s current method, service philosophy, business philosophy, mission, offer, or mature authority. The conclusion must preserve the unresolved assumption that made the familiar life feel more real.',
