...
function publishedPrompt() {
  ...
  const STAGE_CONTRACTS = {
    ...
    7: 'RETURN. Bring the six-part audience canon home through one governing identity transformation that a cold viewer can understand and a returning viewer can recognize. Treat EARLIER SELF from Videos 1-2, FIRST SHIFT from Videos 3-4, FALL from Video 5, and RETURN from Video 6 plus current Video 7 evidence as the complete lived-evidence budget. Develop those four anchors through complete Hook-and-Eye prose without restoring omitted canon, replacing evidence with positioning, or compressing connective logic. Reveal one governing meaning, the earned difference it created, one recognition shared with the viewer as an equal, the honest remainder, and the relational horizon. Do not create a second thesis, new conflict, trial, revelation, service promise, superiority comparison, pitch, or offer.'
  };
  ...
  const QUALITY_REVIEW_SYSTEM = [
    ...
    'For Video 7 at either level, require one connected Return organized around a single governing identity transformation. A cold viewer should understand the earlier person and refusal from Videos 1-2, the actual first realization and trial from Videos 3-4, the fall and human cost from Video 5, and the elixir and returned but unfinished self from Video 6 plus current Video 7 evidence; a returning viewer should recognize the defining callbacks. Treat those four private anchors as the complete canon evidence budget. Each Meat anchor must remain a lived event, choice, consequence, or observable behavior rather than a lesson, identity declaration, positioning claim, service description, or superiority comparison. Reject any Meat event, number, person, example, comparison, or metaphor not selected inside those anchors, along with archive summaries, paragraph-per-video recaps, overloaded chronology, movable retrospective declarations, a Return reduced to one new or present-day scene, or compressed prose that drops the causal connection between selected moments. Preserve independent epiphanies as independent sources when the canon requires it. The Conclusion interprets only evidence already established in Meat and reveals one governing meaning, the earned difference it created, one recognition shared with the viewer as an equal, the honest remainder, and an open relationship. Reject a second thesis, new evidence, market argument, service promise, superiority claim, or promise to teach, show, guide, coach, rescue, transform, or produce a result.',
  ...
  function wholeScriptRewriteMessage(config, script, review, validation) {
    const compositionInstruction = Number(config.video) === 7
      ? 'Apply sentence-level Hook-and-Eye only inside [MEAT]. Treat EARLIER SELF, FIRST SHIFT, FALL, and RETURN as the complete lived-evidence budget, then develop those anchors into complete spoken thoughts without restoring omitted canon, replacing evidence with positioning, or compressing connective logic. Keep the complete visible script inside the global 240-360 word pacing range by removing redundant evidence rather than shortening thoughts. Reserve the one governing meaning in RESERVED RETURN for [CONCLUSION], interpret only evidence already present in [MEAT], and carry one recognition to the viewer as an equal without creating a service promise. Preserve HONEST REMAINDER AND HORIZON, keep [OPEN LOOP] independent from the Hook, and do not imitate wording from the draft below.'
