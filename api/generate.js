...
- RESERVED RETURN states only the one governing meaning created by the four anchors. Write it entirely through the speaker's I/me/my perspective so the final Conclusion does not address or interpret the viewer. It is the brief homecoming statement that will answer the Open Loop. Do not recap an anchor, add another thesis, explain the earned difference again, diagnose the viewer, or introduce a market argument, service description, superiority claim, or unrelated lesson.
...
- HONEST REMAINDER AND HORIZON names the unresolved flaw, fear, habit, limitation, or need that keeps the speaker human, followed by the real direction that makes an ongoing relationship meaningful. The writer will place the honest remainder at the end of MEAT and reserve the relational horizon for CTA; neither belongs in CONCLUSION. Recognition, trust, or continued connection with the speaker must be the primary reason to remain. An unfinished public test may add supporting tension, but watching that test cannot be the viewer's only reason to stay. It cannot become a new central conflict, offer, rescue request, or future episode promise.
...
Preserve CONNECTED JOURNEY PROGRESSION, RESERVED RETURN, HONEST REMAINDER AND HORIZON, and VOICE SIGNALS while creating completely new visible language. Treat EARLIER SELF, FIRST SHIFT, FALL, and RETURN as the complete evidence budget. Develop those lived-evidence anchors into complete spoken thoughts without restoring omitted canon or compressing the prose, let the earned difference emerge inside [MEAT], and place the honest unresolved element at the end of [MEAT]. Let [CONCLUSION] answer the Open Loop in one to three sentences with only the governing meaning from RESERVED RETURN, written entirely in I/me/my without you/your language. It cannot replay an anchor event, recap the four movements, diagnose the viewer, add evidence, or perform another miniature Return. Reserve direct viewer recognition and the continuing relational direction for [CTA]. Apply sentence-level Hook-and-Eye only inside [MEAT]. ${MEAT_COMPOSITION_CONTRACT} Build [OPEN LOOP] independently after [MEAT] and [CONCLUSION] are settled. Supply a provisional [HOOK] for the required format; the global Hook Studio will replace it after the story is finished. Return exactly [HOOK], [OPEN LOOP], [MEAT], [CONCLUSION], and [CTA] with no commentary.`;
    }
    return `${input.userContext}
...
function sectionMessage(input) {
  ...
  const sectionInstruction = isVideoSeven && input.section === 'MEAT'
    ...
    : isVideoSeven && input.section === 'CONCLUSION'
      ? '\n\nVIDEO 7 CONCLUSION REGENERATION REQUIREMENT: Use RESERVED RETURN alone. In one to three sentences, answer the Open Loop with only its governing homecoming meaning and write entirely in I/me/my without you/your language. Do not replay an anchor event, name the four moments again, recap MEAT, address or interpret the viewer, restate the earned difference, include the honest remainder or relational horizon, add evidence, or perform another miniature Return. Reserve direct viewer recognition for CTA.'
