...
- RETURN selects its evidence from Video 6 plus the current Video 7 answers and holds the strongest detail that joins the elixir to observable present behavior. When the elixir came from an independent experience, preserve that independence and connect it through the returned identity rather than false causality.
...
- RESERVED RETURN uses 35-65 words to state one governing meaning created by the four anchors. Within that same meaning, identify what the journey uniquely lets the speaker see or do and one recognition a person in the earlier position can carry. Do not add a second thesis, market argument, service description, comparative superiority claim, or unrelated lesson. For Level 1, center human identity. For Level 2, let the earned professional difference emerge from the selected evidence.
- The peer-to-peer gift returns as recognition shared between equals. Never turn it into a promise that the speaker will teach, show, guide, coach, rescue, transform, or produce a result for the viewer.
- HONEST REMAINDER AND HORIZON uses 20-45 words to name the unresolved flaw, fear, habit, limitation, or need that keeps the speaker human, followed by the real direction that makes an ongoing relationship meaningful. Recognition, trust, or continued connection with the speaker must be the primary relational reason to remain. An unfinished public test may add supporting tension, but watching that test cannot be the viewer's only reason to stay. It cannot become a new central conflict, offer, rescue request, or future episode promise.
...
export async function prepareVideoSevenReturnMaterial(userContext, level, existingScript = '') {
  ...
  return [
    ...
    '',
    'FINAL VIDEO 7 WRITING CONTRACT: Design the Conclusion destination first from RESERVED RETURN together with HONEST REMAINDER AND HORIZON. Build MEAT only from EARLIER SELF, FIRST SHIFT, FALL, and RETURN; never restore omitted canon merely to mention another chapter or example. Treat every anchor as lived evidence rather than a lesson, positioning claim, service description, or comparison. Develop those four anchors into natural, complete spoken thoughts while staying inside the global 240-360 word pacing range for the complete visible script. Apply sentence-level Hook-and-Eye throughout MEAT so each sentence arises from the one before it through cause, consequence, time, escalation, contradiction, recognition, or changed action. Stop before stating the reserved governing meaning or equal-to-equal viewer recognition. The Conclusion interprets only the evidence already present in MEAT, reveals one governing meaning, makes the earned difference visible, and shares one recognition between equals without promising to teach, guide, coach, rescue, or produce a result. Build OPEN LOOP afterward from the exact missing meaning the Conclusion will reveal, without summarizing the journey or disclosing the answer. Supply a provisional HOOK only for formatting; the global Hook Studio replaces it after the complete story is settled. Let the active Video 7 blueprint control the final relational CTA.'
  ].join('\n');
}
...
Preserve CONNECTED JOURNEY PROGRESSION, RESERVED RETURN, HONEST REMAINDER AND HORIZON, and VOICE SIGNALS while creating completely new visible language. Treat EARLIER SELF, FIRST SHIFT, FALL, and RETURN as the complete evidence budget. Develop those lived-evidence anchors into complete spoken thoughts without restoring omitted canon or compressing the prose, and keep the complete visible script inside the global 240-360 word pacing range by removing redundant evidence rather than shortening thoughts. Let [CONCLUSION] interpret only evidence already present in [MEAT], reveal one governing meaning, make the earned difference visible, and share one recognition with the viewer as an equal without becoming a service promise. Apply sentence-level Hook-and-Eye only inside [MEAT]. ${MEAT_COMPOSITION_CONTRACT} Build [OPEN LOOP] independently after [MEAT] and [CONCLUSION] are settled. Supply a provisional [HOOK] for the required format; the global Hook Studio will replace it after the story is finished. Return exactly [HOOK], [OPEN LOOP], [MEAT], [CONCLUSION], and [CTA] with no commentary.`;
    }
    return `${input.userContext}
...
function sectionMessage(input) {
  ...
  const sectionInstruction = isVideoSeven && input.section === 'MEAT'
    ...
    : isVideoSeven && input.section === 'CONCLUSION'
      ? '\n\nVIDEO 7 CONCLUSION REGENERATION REQUIREMENT: Preserve the one governing meaning in RESERVED RETURN together with HONEST REMAINDER AND HORIZON. Interpret only evidence already established in MEAT, make the earned identity or professional difference visible, and carry one recognition to the viewer as an equal. Do not introduce another thesis, revelation, trial, market argument, service promise, superiority comparison, pitch, or offer, and do not promise to teach, show, guide, coach, rescue, or produce a result.'
